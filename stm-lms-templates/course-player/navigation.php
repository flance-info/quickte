	...
	<div class="masterstudy-course-player-navigation__wrapper">
		...
		if ($has_access) {
			...
			?>
					<div class="masterstudy-course-player-navigation__next">
						...
						STM_LMS_Templates::show_lms_template(
							...
							array(
								...
								'style'     => apply_filters('masterstudy_lms_course_player_complete_button_class', $button_style),
								'dark_mode' => $dark_mode,
				...
				'uncompleted' === $data['completed'] && empty($data['next_lesson']) && !$lesson_lock_before_start
				&& !$lesson_locked_by_drip && 'assignments' !== $lesson_type && 'quiz' !== $lesson_type && !empty($current_user['id'])
			) {
				?>
				<div class="masterstudy-course-player-navigation__next">
					<?php
                    if($data['completed' !== 'uncomplete' && current_user_can('administrator') ])
					STM_LMS_Templates::show_lms_template(
						'components/nav-button',
						array(
